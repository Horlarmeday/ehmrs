...
- Security/quality improvements over verbatim (deliberate, documented deviations — frozen surface unchanged): (1) Bearer token attached per-request from a fresh `user_token` localStorage read and omitted entirely when absent — legacy sent `Authorization: Bearer null` on every request when logged out and never refreshed after login/logout in another tab; the interceptor also deletes a stale merged header (the auth store's legacy `delete headers.common['authorization']` uses the wrong casing and never removed it); (2) `NProgress.setColor` validates its argument against a safe color pattern before CSS interpolation; (3) `NProgress.done(true)` immediate on success instead of the legacy 30000ms delay (bar was stuck on screen); (4) unreachable `case 401` in the success switch removed (axios rejects non-2xx; the error handler owns 401). Toast text rendered by PrimeVue as escaped text, so a hostile server `message` cannot XSS.
- Comment policy (user rule, lint-enforced): `local/no-comments` in `eslint.config.js` bans comments across src; allowed exceptions are eslint directives, ALL-CAPS section markers (the legacy route-table section markers survive on this carve-out), triple-slash/shebang, and `! `-prefixed notes (the explicit "necessary" marker). All narrative port/deviation headers were deleted from code — their content lives in this task file; ~230 files trimmed (router header 24→1 line kept as `!` note on the ''-record shadowing, spec headers to one `!` provenance line, 200+ stub-view markers converted to `! ` markers).
- `any`/cast-free guarantee: all 20 direct type casts eliminated (typed runtime guards `asCredentials`/`stringField`/`jsonBody` in `contract/types.ts`, `Reflect.get` in `describeMismatch`, `toMethod` lookup for axios methods, type-guard `hasPath` in the router parity parser, dead legacy root '/' record dropped from the router port — its guard was unreachable in v4 since the '' layout record owns '/'). Enforced going forward by eslint: `@typescript-eslint/no-explicit-any: error` + `no-restricted-syntax` banning `TSAsExpression` (except `as const`) and `TSTypeAssertion` across src; single sanctioned runtime-checked boundary conversion in `jsonBody` carries a justified disable. Negative control verified: a probe file with `5 as string` / `any` fails lint.
