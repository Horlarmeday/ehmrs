...
import { PharmacyDrugType, Status } from '../../database/enums';
...
class StoreService {
  ...
  private static async pharmacyStoreValidations(
    ...
    const [cashItem, nhisItem, privateItem] = await Promise.all([
      getOnePharmacyStoreItem({ drug_id: drugId, drug_type: PharmacyDrugType.CASH }),
      getOnePharmacyStoreItem({ drug_id: drugId, drug_type: PharmacyDrugType.NHIS }),
      getOnePharmacyStoreItem({ drug_id: drugId, drug_type: PharmacyDrugType.PRIVATE }),
    ]);
