...
export const validateCreateAudit = validate(
  Joi.object({
    store_type: Joi.string().valid('PHARMACY', 'LABORATORY').required(),
    initiated_by: Joi.number().required(),
    inventory_ids: Joi.array().items(Joi.number()).min(1).required(),
...
export const validateUpdateAudit = validate(
  Joi.object({
    notes: Joi.string().allow('').optional(),
    status: Joi.string().valid('Draft', 'In Progress', 'Completed', 'Approved', 'Cancelled').optional(),
  })
);
...
export const validateCompleteAudit = validate(
  Joi.object({
    ...
    completion_notes: Joi.string().allow('').optional(),
  })
);
