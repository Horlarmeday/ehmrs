import Joi from 'joi';
import { ItemStatus, MovementType, RequestStatus, Priority, ItemRequestStatus } from '../../database/models/generalStore/types';
...
export const createCategorySchema = Joi.object({
  ...
  parent_id: Joi.number().optional().positive().messages({
...
export const updateSubcategorySchema = Joi.object({
  ...
  category_id: Joi.number().optional().positive().messages({
    'number.base': 'Category ID must be a valid number',
    'number.positive': 'Category ID must be a positive number'
  }),
  is_active: Joi.boolean().optional()
...
export const createItemSchema = Joi.object({
  item_code: Joi.string().required().min(3).max(20).pattern(/^[A-Z0-9-]+$/).messages({
  ...
  manufacturer: Joi.string().optional().max(200).messages({
    'string.max': 'Manufacturer name cannot exceed 200 characters'
  }),
  model_number: Joi.string().optional().max(100).messages({
    'string.max': 'Model number cannot exceed 100 characters'
  }),
  specifications: Joi.string().optional().max(2000).messages({
    'string.max': 'Specifications cannot exceed 2000 characters'
  }),
  minimum_stock: Joi.number().required().min(0).max(999999).messages({
  ...
  maximum_stock: Joi.number().required().min(1).max(999999).messages({
  ...
  unit_cost: Joi.number().required().min(0).max(999999.99).precision(2).messages({
  ...
  shelf_number: Joi.string().optional().max(50).messages({
    'string.max': 'Shelf number cannot exceed 50 characters'
  }),
  expiry_date: Joi.date().optional().greater('now').messages({
    'date.greater': 'Expiry date must be in the future'
  }),
  is_expirable: Joi.boolean().required().messages({
    'any.required': 'Expirable flag is required'
  }),
  is_serialized: Joi.boolean().required().messages({
    'any.required': 'Serialized flag is required'
  }),
  is_lot_tracked: Joi.boolean().required().messages({
    'any.required': 'Lot tracked flag is required'
  }),
  supplier_id: Joi.number().optional().positive().messages({
...
export const updateItemSchema = Joi.object({
  item_code: Joi.string().optional().min(3).max(20).pattern(/^[A-Z0-9-]+$/).messages({
    'string.min': 'Item code must be at least 3 characters long',
    'string.max': 'Item code cannot exceed 20 characters',
    'string.pattern.base': 'Item code must contain only uppercase letters, numbers, and hyphens'
  }),
  ...
  subcategory_id: Joi.number().optional().positive().messages({
    'number.base': 'Subcategory ID must be a valid number',
    'number.positive': 'Subcategory ID must be a positive number'
  }),
  unit_id: Joi.number().optional().positive().messages({
    'number.base': 'Unit ID must be a valid number',
    'number.positive': 'Unit ID must be a positive number'
  }),
  manufacturer: Joi.string().optional().max(200).messages({
  ...
  minimum_stock: Joi.number().optional().min(0).max(999999).messages({
    'number.base': 'Minimum stock must be a valid number',
    'number.min': 'Minimum stock cannot be negative',
    'number.max': 'Minimum stock cannot exceed 999,999'
  }),
  maximum_stock: Joi.number().optional().min(1).max(999999).messages({
    'number.base': 'Maximum stock must be a valid number',
    'number.min': 'Maximum stock must be at least 1',
    'number.max': 'Maximum stock cannot exceed 999,999'
  }),
  unit_cost: Joi.number().optional().min(0).max(999999.99).precision(2).messages({
    'number.base': 'Unit cost must be a valid number',
    'number.min': 'Unit cost cannot be negative',
    'number.max': 'Unit cost cannot exceed 999,999.99',
    'number.precision': 'Unit cost can have maximum 2 decimal places'
  }),
  location: Joi.string().optional().min(2).max(100).messages({
    'string.min': 'Location must be at least 2 characters long',
    'string.max': 'Location cannot exceed 100 characters'
  }),
  shelf_number: Joi.string().optional().max(50).messages({
  ...
  status: Joi.string().optional().valid(...Object.values(ItemStatus)).messages({
...
export const createMovementSchema = Joi.object({
  ...
  reference_type: Joi.string().required().min(2).max(50).messages({
  ...
  reference_id: Joi.number().required().positive().messages({
  ...
  from_location: Joi.string().optional().max(100).messages({
    'string.max': 'From location cannot exceed 100 characters'
  }),
  to_location: Joi.string().optional().max(100).messages({
...
export const createRequestSchema = Joi.object({
  requesting_department: Joi.string().required().min(2).max(100).messages({
  ...
  priority: Joi.string().required().valid(...Object.values(Priority)).messages({
    'any.only': 'Priority must be one of: LOW, MEDIUM, HIGH, URGENT',
    'any.required': 'Priority is required'
  }),
  required_date: Joi.date().required().greater('now').messages({
  ...
  items: Joi.array().required().min(1).max(100).items(
    Joi.object({
      item_id: Joi.number().required().positive().messages({
        'number.base': 'Item ID must be a valid number',
        'number.positive': 'Item ID must be a positive number',
      ...
      quantity_requested: Joi.number().required().positive().max(999999).messages({
...
export const approveRequestSchema = Joi.object({
  approved_items: Joi.array().required().min(1).items(
    Joi.object({
      item_id: Joi.number().required().positive().messages({
        'number.base': 'Item ID must be a valid number',
        'number.positive': 'Item ID must be a positive number',
      ...
      quantity_approved: Joi.number().required().positive().max(999999).messages({
...
export const fulfillRequestSchema = Joi.object({
  issued_items: Joi.array().required().min(1).items(
    Joi.object({
      item_id: Joi.number().required().positive().messages({
        'number.base': 'Item ID must be a valid number',
        'number.positive': 'Item ID must be a positive number',
      ...
      quantity_issued: Joi.number().required().positive().max(999999).messages({
...
export const itemFilterSchema = Joi.object({
  ...
  search: Joi.string().optional().min(2).max(100).messages({
...
export const movementFilterSchema = Joi.object({
  item_id: Joi.number().optional().positive().messages({
    'number.base': 'Item ID must be a valid number',
    'number.positive': 'Item ID must be a positive number'
  }),
  movement_type: Joi.string().optional().valid(...Object.values(MovementType)).messages({
    'any.only': 'Movement type must be one of: IN, OUT, TRANSFER, ADJUSTMENT'
  }),
  ...
  staff_id: Joi.number().optional().positive().messages({
...
export const requestFilterSchema = Joi.object({
  status: Joi.string().optional().valid(...Object.values(RequestStatus)).messages({
    'any.only': 'Status must be one of: PENDING, APPROVED, REJECTED, FULFILLED, PARTIALLY_FULFILLED'
  }),
  priority: Joi.string().optional().valid(...Object.values(Priority)).messages({
    'any.only': 'Priority must be one of: LOW, MEDIUM, HIGH, URGENT'
  }),
  requesting_department: Joi.string().optional().min(2).max(100).messages({
    'string.min': 'Department name must be at least 2 characters long',
    'string.max': 'Department name cannot exceed 100 characters'
  }),
  start_date: Joi.date().optional().messages({
...
export const reportFilterSchema = Joi.object({
  ...
  group_by: Joi.string().optional().valid('category', 'subcategory', 'supplier').messages({
