...
export const createHMOClaimSchema = Joi.object({
  bill_id: Joi.number().integer().min(1).required().messages({
  ...
  patient_id: Joi.number().integer().min(1).required().messages({
  ...
  hmo_id: Joi.number().integer().min(1).required().messages({
  ...
  claim_number: Joi.string().required().max(100).messages({
  ...
  claim_amount: Joi.number().precision(2).min(0).required().messages({
    'number.base': 'Claim amount must be a number',
    'number.min': 'Claim amount cannot be negative',
    'any.required': 'Claim amount is required',
  }),
  approved_amount: Joi.number().precision(2).min(0).optional().default(0).messages({
    'number.base': 'Approved amount must be a number',
    'number.min': 'Approved amount cannot be negative',
  }),
  status: Joi.string().valid('PENDING', 'SUBMITTED', 'APPROVED', 'REJECTED', 'PAID').optional().default('PENDING').messages({
    'any.only': 'Status must be one of: PENDING, SUBMITTED, APPROVED, REJECTED, PAID',
  }),
  submission_date: Joi.string().isoDate().optional().messages({
...
export const updateHMOClaimSchema = Joi.object({
  bill_id: Joi.number().integer().min(1).optional().messages({
    'number.base': 'Bill ID must be a number',
    'number.integer': 'Bill ID must be an integer',
    'number.min': 'Bill ID must be greater than 0',
  }),
  ...
  hmo_id: Joi.number().integer().min(1).optional().messages({
    'number.base': 'HMO ID must be a number',
    'number.integer': 'HMO ID must be an integer',
    'number.min': 'HMO ID must be greater than 0',
  }),
  claim_number: Joi.string().max(100).optional().messages({
    'string.max': 'Claim number cannot exceed 100 characters',
  }),
  claim_amount: Joi.number().precision(2).min(0).optional().messages({
    'number.base': 'Claim amount must be a number',
    'number.min': 'Claim amount cannot be negative',
  }),
  approved_amount: Joi.number().precision(2).min(0).optional().messages({
    'number.base': 'Approved amount must be a number',
    'number.min': 'Approved amount cannot be negative',
  }),
  status: Joi.string().valid('PENDING', 'SUBMITTED', 'APPROVED', 'REJECTED', 'PAID').optional().messages({
    'any.only': 'Status must be one of: PENDING, SUBMITTED, APPROVED, REJECTED, PAID',
  }),
  submission_date: Joi.string().isoDate().optional().messages({
    'string.isoDate': 'Submission date must be a valid date',
  }),
  approval_date: Joi.string().isoDate().optional().messages({
    'string.isoDate': 'Approval date must be a valid date',
  }),
  payment_date: Joi.string().isoDate().optional().messages({
    'string.isoDate': 'Payment date must be a valid date',
  }),
  notes: Joi.string().max(1000).optional().messages({
    'string.max': 'Notes cannot exceed 1000 characters',
  }),
  rejection_reason: Joi.string().max(1000).optional().messages({
