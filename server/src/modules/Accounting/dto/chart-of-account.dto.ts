...
export const createChartOfAccountSchema = Joi.object({
  ...
  type: Joi.string().required().valid('ASSET', 'LIABILITY', 'EQUITY', 'INCOME', 'EXPENSE').messages({
  ...
  tax_code: Joi.string().max(20).optional().allow('').messages({
    'string.max': 'Tax code cannot exceed 20 characters',
  }),
  budget_allocation: Joi.number().precision(2).min(0).optional().default(0).messages({
    'number.base': 'Budget allocation must be a number',
    'number.min': 'Budget allocation cannot be negative',
  }),
  allow_manual_entries: Joi.boolean().optional().default(true).messages({
...
export const updateChartOfAccountSchema = Joi.object({
  ...
  type: Joi.string().valid('ASSET', 'LIABILITY', 'EQUITY', 'INCOME', 'EXPENSE').optional().messages({
    'any.only': 'Account type must be one of: ASSET, LIABILITY, EQUITY, INCOME, EXPENSE',
  }),
  parent_id: Joi.number().integer().min(1).allow(null).optional().messages({
    'number.base': 'Parent ID must be a number',
  ...
  budget_allocation: Joi.number().precision(2).min(0).optional().messages({
    'number.base': 'Budget allocation must be a number',
    'number.min': 'Budget allocation cannot be negative',
  }),
  allow_manual_entries: Joi.boolean().optional().messages({
    'boolean.base': 'allow_manual_entries must be a boolean',
  }),
});
