...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    try {
      ...
      console.log(`Found ${drugs.length} drugs, ${insurances.length} insurances, ${tests.length} tests, ${services.length} services, ${investigations.length} investigations`);
      ...
      if (hmoServicePricingData.length > 0) {
        await queryInterface.bulkInsert('HMO_Service_Pricing', hmoServicePricingData, { transaction });
      ...
      if (hmoInvestigationPricingData.length > 0) {
        await queryInterface.bulkInsert('HMO_Investigation_Pricing', hmoInvestigationPricingData, { transaction });
        console.log(`Inserted ${hmoInvestigationPricingData.length} HMO investigation pricing records`);
      }
