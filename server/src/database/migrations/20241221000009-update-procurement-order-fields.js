...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    try {
      ...
      // Add sent_date field
      await queryInterface.addColumn('Procurement_Orders', 'sent_date', {
        type: Sequelize.DATE,
        allowNull: true,
      }, { transaction });

      // Add received_date field
      await queryInterface.addColumn('Procurement_Orders', 'received_date', {
        type: Sequelize.DATE,
        allowNull: true,
      }, { transaction });

      // Add cancelled_date field
      await queryInterface.addColumn('Procurement_Orders', 'cancelled_date', {
        type: Sequelize.DATE,
        allowNull: true,
      }, { transaction });

      // Add cancellation_reason field
      await queryInterface.addColumn('Procurement_Orders', 'cancellation_reason', {
        type: Sequelize.TEXT,
        allowNull: true,
      }, { transaction });
  ...
  down: async (queryInterface, Sequelize) => {
    ...
    try {
      ...
      await queryInterface.removeColumn('Procurement_Orders', 'cancellation_reason', { transaction });
