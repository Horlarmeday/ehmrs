...
async function createTestData() {
  ...
  try {
    ...
    const bill = await ClinicalBill.create({
      ...
      patient_co_pay_amount: 100.0,
      hmo_billed_amount: 900.0,
    ...
    const cashRegister = await CashRegister.create({
      ...
      expected_closing_balance: 5000.0,
      actual_closing_balance: 5000.0,
...
async function testDataRetrieval(testData, paymentResults) {
  ...
  try {
    ...
    const insurancePaymentWithData = await ClinicalPayment.findByPk(
      paymentResults.insurancePayment.id,
      {
    ...
    if (posPaymentWithData && posPaymentWithData.posTerminalTransaction) {
      ...
      log(
        `Terminal Location: ${posPaymentWithData.posTerminalTransaction.terminal?.location}`,
        'success'
      );
    } else {
