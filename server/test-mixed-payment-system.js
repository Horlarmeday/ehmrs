...
async function createTestData() {
  ...
  try {
    ...
    const bill = await ClinicalBill.create({
      ...
      patient_co_pay_amount: 200.0,
      hmo_billed_amount: 1800.0,
...
async function testMixedPaymentQueries(testData, paymentResults) {
  ...
  try {
    ...
    if (mixedPayments.length > 0) {
      ...
      const hasMixPrefix = mixedPayments.every(
        p => p.payment_reference && p.payment_reference.startsWith('MIX-')
      );
      if (hasMixPrefix) {
