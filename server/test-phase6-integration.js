...
async function testPhase6Integration() {
  ...
  try {
    ...
    const comprehensiveReport = await testEndpoint('/accounting/reports/financial/comprehensive', testParams);
    ...
    const balanceSheet = await testEndpoint('/accounting/reports/financial/balance-sheet', testParams);
    ...
    const operationalPerformance = await testEndpoint('/accounting/reports/operational/performance', testParams);
    console.log('✅ Operational Performance:', operationalPerformance ? 'SUCCESS' : 'FAILED');
    
    const paymentUtilization = await testEndpoint('/accounting/reports/operational/utilization', testParams);
    console.log('✅ Payment Method Utilization:', paymentUtilization ? 'SUCCESS' : 'FAILED');
    
    const reconciliationStatus = await testEndpoint('/accounting/reports/operational/reconciliation', testParams);
    console.log('✅ Reconciliation Status:', reconciliationStatus ? 'SUCCESS' : 'FAILED');
    
    const settlementTracking = await testEndpoint('/accounting/reports/operational/settlement', testParams);
    ...
    const comprehensiveBI = await testEndpoint('/accounting/reports/business-intelligence/comprehensive', testParams);
    console.log('✅ Comprehensive BI Report:', comprehensiveBI ? 'SUCCESS' : 'FAILED');
    
    const trendAnalysis = await testEndpoint('/accounting/reports/business-intelligence/trends', testParams);
    console.log('✅ Payment Trend Analysis:', trendAnalysis ? 'SUCCESS' : 'FAILED');
    
    const predictiveAnalytics = await testEndpoint('/accounting/reports/business-intelligence/predictive', testParams);
    console.log('✅ Predictive Analytics:', predictiveAnalytics ? 'SUCCESS' : 'FAILED');
    
    const kpiMonitoring = await testEndpoint('/accounting/reports/business-intelligence/kpi', testParams);
    console.log('✅ KPI Monitoring:', kpiMonitoring ? 'SUCCESS' : 'FAILED');
    
    const realTimeMonitoring = await testEndpoint('/accounting/reports/business-intelligence/real-time', testParams);
...
async function testEndpoint(endpoint, params) {
  try {
    ...
    if (error.response) {
      console.log(`⚠️  ${endpoint}: ${error.response.status} - ${error.response.data.message || 'Unknown error'}`);
    } else {
...
async function testServiceMethods() {
  ...
  try {
    ...
    console.log('ℹ️  Use: node -e "require(\'./src/modules/Accounting/services/financialReporting.service\')"');
