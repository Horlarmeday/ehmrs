...
export default {
  ...
  methods: {
    ...
    handleDispenseDrug(event) {
      const { prescription, index, quantity, collectedBy } = event;
      const obj = {
    ...
    handleDispenseItem(event) {
      const { item, index, quantity, collectedBy } = event;
      const obj = {
...
.history-btn:hover {
  ...
  box-shadow: 0 6px 20px 0 rgba(139, 0, 0, 0.4);
}
...
.save-btn:not(:disabled):hover {
  ...
  box-shadow: 0 6px 20px 0 rgba(0, 0, 128, 0.4);
}
...
.complete-btn:not(:disabled):hover {
  ...
  box-shadow: 0 6px 20px 0 rgba(0, 100, 0, 0.4);
}
...
.status-card.active {
  background: linear-gradient(135deg, #f0fff0 0%, #90ee90 100%);
  border-color: #006400;
}
