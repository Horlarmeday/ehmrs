<template>
  <div>
        ...
        <table class="table table-head-custom table-head-bg table-vertical-center">
          <thead>
            <tr class="text-uppercase">
              ...
              <th style="min-width: 120px">Ward / Bed</th>
          ...
          <tbody v-if="samples.length === 0">
            <tr>
              <td colspan="11" align="center" class="text-muted">No Data</td>
            </tr>
          </tbody>
            ...
            <tr :class="{ disabled: sample.total === sample.total_pending_payments }">
              ...
              <td>
                <span class="text-dark-75 font-weight-bolder d-block font-size-md">
                  {{ sample.ward }}{{ sample.bed ? ` / ${sample.bed}` : '' }}
                </span>
              </td>
              <td>
                <span class="text-dark-75 font-weight-bolder d-block font-size-md">
              ...
              <td>
                <span class="text-dark-75 font-weight-bolder d-block font-size-md">{{
                  sample.date_requested | dayjs('DD/MM/YYYY, h:mma')
                }}</span>
              </td>
              <td class="text-right pr-0">
                <router-link
                  :class="{ disabled: sample.status !== 'Pending' }"
                  v-b-tooltip.hover
    ...
    <table-skeleton v-else :columns="11" />
  </div>
</template>
...
export default {
  ...
  computed: {
    samples() {
      return this.$store.state.laboratory.admittedPatientsSamples;
    },
    queriedItems() {
      return this.$store.state.laboratory.totalAdmittedPatientsSamples;
    },
    pages() {
      return this.$store.state.laboratory.totalAdmittedPatientsSamplesPages;
    },
    perPage() {
  ...
  methods: {
    ...
    handlePageChange() {
      ...
      this.fetchAdmittedPatientsSamples({
    ...
    debounceSearch: debounce((search, vm, spinDiv) => {
      vm.fetchAdmittedPatientsSamples({
        currentPage: 1,
    ...
    fetchAdmittedPatientsSamples({ currentPage, itemsPerPage, search, start, end }) {
      return this.$store.dispatch('laboratory/fetchAdmittedPatientsSamples', {
        currentPage,
        itemsPerPage,
        ...(search && { search }),
        ...(start && end && { start, end }),
