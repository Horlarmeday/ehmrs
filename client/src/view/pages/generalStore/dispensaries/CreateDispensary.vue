<template>
  <div class="create-dispensary">
    <div class="row">
      <div class="col-12">
        <div class="card card-custom gutter-b">
          <div class="card-header border-0 py-5">
            <h3 class="card-title align-items-start flex-column">
              <span class="card-label font-weight-bolder text-dark">Create New Dispensary</span>
              <span class="text-muted mt-3 font-weight-bold font-size-sm">
                Add a new dispensary to the general store system
              </span>
            </h3>
                ...
                <div class="col-lg-4">
                  ...
                  <div class="card card-custom bg-light-primary">
                    <div class="card-body">
                      ...
                      <div class="text-dark">
                        <p class="mb-2">
                          <strong>Name:</strong> Choose a clear, descriptive name for the
                          dispensary.
                        </p>
                        <p class="mb-2">
                          <strong>Location:</strong> Specify the physical location within the
                          hospital.
                        </p>
                        <p class="mb-2">
                          <strong>Auto-replenishment:</strong> When enabled, the system will
                          automatically request stock transfers when items fall below the threshold.
                        </p>
                        <p class="mb-0">
                          <strong>Status:</strong> Only active dispensaries can receive transfers
                          and dispense items.
                        </p>
                      </div>
                    </div>
...
export default {
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      try {
        await this.$store.dispatch('generalStore/createDispensary', this.formData);

        this.$toast.success('Dispensary created successfully');
        this.$router.push('/general-store/dispensaries');
      } catch (error) {
        this.$toast.error(error.response?.data?.message || 'Failed to create dispensary');
      } finally {
        this.loading = false;
      }
    },
